...
#include "..\Env\EnvDodgeEffect.cginc"
...
fixed4 frag (v2f i) : SV_Target
{
	...
	finalColor = SetHeightDissovle(finalColor,i.uv,worldPos);
	#endif

	finalColor.rgb = EnvDodgeEffectChangeColor(finalColor.rgb, worldPos);
	return finalColor;
}
...
v2f veryLowVert(low_appdata v)
{
	...
	o.pos = UnityObjectToClipPos(v.vertex);
	o.uv = TRANSFORM_TEX(v.uv, _MainTex);
	o.vertexColor = mul(unity_ObjectToWorld, v.vertex);
	return o;
}
...
fixed4 veryLowfrag (v2f i) : SV_Target
{
	...
	finalColor.rgb = EnvDodgeEffectChangeColor(finalColor.rgb, i.vertexColor);
	//finalColor = SetEmmisveColor(i,lightMaskCol,finalColor);
	return finalColor;
}