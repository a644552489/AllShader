...
fixed _FabricScatterScale;
fixed _FabricScatterScaleFromTex;
...
half3 FabricScatterItem(float nl, float nv, half3 metallicPart)
{
	float tmp = step(0.1, _FabricScatterScaleFromTex);
	return tmp * _FabricScatterColor * (nl*0.5 + 0.5) * FabricScatterFresnelLerp(nv, _FabricScatterScale * _FabricScatterScaleFromTex)
			+ (1 - tmp) * metallicPart;
}
#endif
...
#   endif
	...
	half3 color =   diffColor * (gi.diffuse + light.color * diffuseTerm)
                    ...
                    + FabricScatterItem(nl, nv, surfaceReduction * gi.specular * FresnelLerp (specColor, grazingTerm, nv));
#else
	half3 color =   diffColor * (gi.diffuse + light.color * diffuseTerm)
					+ specularTerm * light.color * FresnelTerm (specColor, lh)
...
//#endif
	...
	half3 color =   (diffColor + specularTerm * specColor) * light.color * nl
					...
					+ FabricScatterItem(nl, nv, surfaceReduction * gi.specular * FresnelLerpFast (specColor, grazingTerm, nv));
...
half3 BRDF3_Indirect1(half3 diffColor, half3 specColor, UnityIndirect indirect, half grazingTerm, half fresnelTerm, float nl, float nv)
{
	half3 c = indirect.diffuse * diffColor;
#if _FABRIC
	c += FabricScatterItem(nl, nv, indirect.specular * lerp (specColor, grazingTerm, fresnelTerm));
#else
	...
	UnityLight light, UnityIndirect gi)
{
	...
	color += BRDF3_Indirect1(diffColor, specColor, gi, grazingTerm, fresnelTerm, nl, nv);
...
half3 MetallicGloss(float2 uv)
{
    ...
    #if defined(_FAKELINEAR)
        mg = tex2D(_MetallicGlossMap, uv).rab;
...
{
    half3 metallicGloss = MetallicGloss(i_tex.xy);
    half metallic = metallicGloss.x;
    half smoothness = metallicGloss.y; // this is 1 minus the square root of real roughness m.
	_FabricScatterScaleFromTex = metallicGloss.z;
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	half normalDotEye = saturate(dot(s.normalWorld, mainLight.dir));
	half falloffU = clamp( 1.0 - abs( normalDotEye ), 0.02, 0.98 );
	half4 falloffSamplerColor = tex2D( _FallOffMap, float2( falloffU, 0.25f ) );
	s.diffColor *= (1 + _FallOffScale*(_FallOffDelta - falloffSamplerColor.r) );
    ...
    #endif
	// Rimlight
	falloffU = 1 - saturate(  dot( s.normalWorld, -s.eyeVec )  );
	half rimlightDot = 1-saturate(  dot( s.normalWorld, mainLight.dir )  );
	falloffU = saturate( rimlightDot * falloffU );
	falloffU = tex2D( _RimLightMap, float2( saturate(falloffU+_RimEdgeDelta), 0.25f ) ).r;
	half3 lightColor = s.diffColor.rgb; // * 2.0;
    half modelHeightFactor = i.pos.y/_ScreenParams.y;
	c.rgb = lerp(c.rgb, _RimLightColor.rgb, falloffU * _RimLightColor.a * modelHeightFactor * occlusion * _RimStrength);
