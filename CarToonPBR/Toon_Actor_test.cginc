...
fixed4 frag (v2f i) : SV_Target
{
	...
	fixed3 LightColor = lerp(_LightColor0, _FixedLightColor.rgb, _IsUseFixedLight);
	...
	half normalDotLight = saturate(dot(worldNormal, LightDir));
	half falloffU = clamp( 1.0 - abs( normalDotLight ), 0.02, 0.98 );
	half4 falloffSamplerColor = tex2D( _FallOffMap, float2( falloffU, 0.25f ) );

	LightColor *= (1 + _FallOffScale*(_FallOffDelta - falloffSamplerColor.r) );
//return float4(falloffSamplerColor.rgb, 1);
	//half3 shadowColor = mainCol.rgb * mainCol.rgb;
//	half3 combinedColor = lerp( mainCol.rgb, shadowColor, falloffSamplerColor.r );
//	combinedColor *= ( 1.0 + falloffSamplerColor.rgb * falloffSamplerColor.a );
//return float4(combinedColor.rgb, 1);

	#if _USE_RIM_LIGHT_ON_V2
		// Rimlight
		falloffU = 1 - saturate(dot(worldNormal, viewDir));
		normalDotLight = 1 - normalDotLight;
		falloffU = saturate( normalDotLight * falloffU );
		falloffU = tex2D( _RimLightMap, float2( saturate(falloffU+_RimEdgeDelta), 0.25f ) ).r;
		half modelHeightFactor = i.pos.y/_ScreenParams.y;
		half3 lightColor = _RimLightColor.rgb * _RimLightColor.a * _RimStrengthV2 * modelHeightFactor; // * 2.0;
		finalColorRGB += falloffU * lightColor;
	#endif
	...
	finalColor.a =_BodyAlPah * saturate(mainCol.a+_TexAlaphDelta);
//return float4(float3(mainCol.a,mainCol.a, mainCol.a), 1);
	return finalColor;
}
