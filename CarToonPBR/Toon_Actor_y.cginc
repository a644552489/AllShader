...
sampler2D _CustomTex; //自定义贴图01   
	// r:高光明度贴图 区分不同材质不同明度（可当高光mask用 因为纯黑的地方将被强制不显示高光） 
	// g:亮部信息（接近纯白即可）
	// b:高光信息（ 1.控制高光外形2.控制高光范围（由shininess强度按白到黑控制高光外形和逐渐出现高光信息））
...
fixed4 frag(v2f i) : SV_Target
{
	...
	//阴影颜色 乘 主贴图
	fixed4 CustomTex = tex2D(_CustomTex, i.uv);
	...
	*/
	 ...
	 //=====================================重新定义高光部分=================================
      fixed specular = pow(max(0, dot(worldNormal, halfDir)), _Gloss *4);  // *256  _Gloss  150* CustomTex.a
   fixed oneMinusSpec = CustomTex.b;              //反向高光信息  CustomTex.b
   oneMinusSpec *= specular;
  // fixed specMaslk = smoothstep(0, _SpecularSmooth,oneMinusSpec);  //_SpecularSmooth
   half3 finalSpecularCol = _SpecularColor.rgb;
  // finalSpecularCol *= CustomTex.r ;
   finalSpecularCol += finalSpecularCol * mainCol * _ShinnessMulti;
   finalSpecularCol *= oneMinusSpec;

		//===============================漫反射======================================
		//贴图 R 通道 *  顶点 R通道 共同控制
		fixed diffuseMask = (halfLambert + (1-lightMaskCol.r ) * i.vertexColor.r) * 0.5;
		...
		diffuseMask -= _LightArea;
		//int diffuseStep = step(0, diffuseMask);
		fixed diffuseStep = smoothstep(0, _ShadowWidthSmooth, saturate(diffuseMask));
		half3 finalDiffuseColor = lerp(shadowCol.rgb,mainCol.rgb,diffuseStep);
		finalDiffuseColor = lerp(finalDiffuseColor,_HighlightColor,  CustomTex.g);   //亮部设置
